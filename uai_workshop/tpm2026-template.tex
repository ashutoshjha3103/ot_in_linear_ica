...
\begin{abstract}
Independent Component Analysis (ICA) recovers mutually independent sources from linear mixtures. Classical algorithms---FastICA, JADE, and InfoMax---compute non-Gaussianity via surrogate contrast functions (negentropy approximations, fourth-order cumulants, parametric log-likelihoods) that fail on heterogeneous source distributions. We propose OT-ICA, which uses the 1D squared Wasserstein distance ($W_2^2$) to a standard Gaussian as the contrast function, computed exactly by quantile sorting without density estimation. We prove $W_2^2$ satisfies a weighted mixture upper bound with strict inequality for non-Gaussian sources, establishing it as a valid ICA contrast. Across $d \in \{10, 20, 30\}$ and five mixture regimes ($N = 10{,}000$, 10 trials), OT-ICA achieves lower Amari error than FastICA, JADE, InfoMax, and Picard on all continuous and heterogeneous distributions. On purely discrete sources the contrast is present, but step-function CDFs produce gradient plateaus that stall the solver. Two applications are presented: ocular artifact removal from frontal EEG, and structural shock identification in Vector Error Correction Models for financial price discovery.
\end{abstract}

\section{Introduction}
Independent Component Analysis (ICA) \citep{jutten1985, comon1994} recovers mutually independent source signals $\mathbf{S}$ from a linear mixture $\mathbf{X} = \mathbf{A}\mathbf{S}$. Identifiability holds under the following conditions \citep{comon1994}.

\begin{theorem}[Linear ICA Identifiability]
Let $\mathbf{X} \in \mathbb{R}^d$ be observed data and $\mathbf{S} \in \mathbb{R}^d$ latent sources. Assume (i) $\mathbf{X} = \mathbf{A}\mathbf{S}$ with $\mathbf{A}$ invertible, (ii) $s_1, \dots, s_d$ mutually independent, and (iii) at most one $s_i$ Gaussian. Then $\mathbf{A}$ is identifiable up to column permutation and scaling.
\end{theorem}

The Central Limit Theorem implies that linear mixtures of independent non-Gaussian sources are more Gaussian than their constituents. ICA recovers sources by maximizing marginal non-Gaussianity over the orthogonal group \citep{hyvarinen2001}, justified by the following identity.
...
\end{equation}
where $G(\cdot)$ denotes KL divergence to the best-fitting Gaussian. Since $G(Y)$ is invariant under orthogonal rotation, minimizing $\mathscr{I}(Y)$ is equivalent to maximizing $\sum_i G(Y_i)$.
\end{theorem}
\textit{(Proof via KL Pythagorean identities on the Product and Gaussian manifolds: Appendix~\ref{app:info_geometry}.)}

Evaluating $G(Y_i)$ exactly requires the marginal density. Classical algorithms replace it with surrogates: FastICA \citep{hyvarinen2001, amari1996new} uses logcosh negentropy approximations, JADE \citep{cardoso1993blind} fourth-order cumulants, InfoMax \citep{bell1995information} a parametric log-likelihood, and Picard \citep{ablin2018faster} an adaptive nonlinearity with a provably converging solver. Each proxy fails on specific source geometries (Appendix~\ref{app:proxy_limits}).

We propose OT-ICA, using $W_2^2$ to a standard Gaussian as the contrast. The Monge--Kantorovich problem \citep{monge1781, kantorovich1942} defines the $p$-Wasserstein distance between measures $\mu$ and $\nu$ as:
\begin{equation}
...
\end{equation}
This is computable exactly by quantile sorting. Prior work uses 1D $W_2$ for normality testing \citep{delbarrio1999} and distribution comparison \citep{peyre2019computational}; we formalize $W_2^2$-to-Gaussian as a valid ICA contrast with finite-sample convergence guarantees.

\section{The OT-ICA Framework}

After centering and whitening $\mathbf{X}$ to $\widetilde{\mathbf{X}}$, the unmixing matrix is constrained to the Orthogonal Group $\mathcal{S}$ \citep{cardoso2022}. OT-ICA finds the projection $\mathbf{b}$ maximizing $W_2^2$ to the standard Gaussian $\Gamma \equiv \mathcal{N}(0,1)$:
\begin{equation}
...
The global maximum is attained when $\mathbf{b}$ aligns with a single source direction. Proofs via comonotonicity contradictions under Caffarelli's regularity theory are in Appendix~\ref{app:proofs}.

\subsection{Methodological Enhancements}
Three components are required for optimization on $\mathcal{S}$: (i)~\textbf{Analytical Gaussian Targets} \citep{fournier2015rate}, replacing sampled quantile targets with their closed-form bin expectations to eliminate approximation noise; (ii)~\textbf{Riemannian Gradient Retraction} \citep{absil2008optimization}, projecting gradients onto the tangent space of $\mathcal{S}$ and retracting via symmetric decorrelation; and (iii)~\textbf{Gaussian Dithering} \citep{schuchman1964dither}, convolving discrete empirical distributions with a narrow Gaussian kernel to smooth step-function CDFs. Full formulations and pseudocode are in Appendix~\ref{app:methodology}.

\section{OT-ICA Methodology Validation}
Separation quality is measured by the Amari Performance Index ($E$) \citep{amari1996new}, the deviation of the gain matrix $\mathbf{P} = \mathbf{B}_{\text{est}}\mathbf{A}$ from a generalized permutation matrix ($E < 0.1$: near-perfect; $E \ge 0.5$: failure; definition in Appendix~\ref{app:experimental_setup}).
...
Figure~\ref{fig:ot_ica_validation} shows $W_2^2$ driving the Amari error to near zero on Laplace, Uniform, Student-$t$, and Beta sources, confirming that the Riemannian solver with analytical Gaussian targets converges reliably on continuous distributions.
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.98\textwidth]{general_hybrid_test.pdf}
    \caption{Mean Amari error ($\downarrow$ better, clipped at $1.5$) across five mixture regimes and $d \in \{10, 20, 30\}$ ($N=10{,}000$, 10 trials, 95\% CI). OT-ICA achieves the lowest error on all continuous and heterogeneous regimes at every dimension. On \textit{Discrete Only}, the $W_2^2$ contrast is present but step-function CDFs produce gradient plateaus that stall the solver (Table~\ref{tab:w2_sensitivity}, Appendix~\ref{app:discrete_contrast}). At $d=30$, all proxy-based methods saturate the clip ceiling on discrete and several heterogeneous regimes.}
    \label{fig:hybrid_mixture_results}
\end{figure*}

We compare OT-ICA against FastICA \citep{hyvarinen2001}, JADE \citep{cardoso1993blind}, InfoMax \citep{bell1995information}, and Picard \citep{ablin2018faster} on five mixture regimes at $d \in \{10, 20, 30\}$, $N = 10{,}000$, 10 trials per condition (Figure~\ref{fig:hybrid_mixture_results}).

\textbf{Continuous and heterogeneous regimes.} OT-ICA achieves the lowest Amari error on all four continuous and mixed-type configurations at every dimension. On \textit{Continuous Only} sources, OT-ICA attains $E = 0.051$, $0.107$, $0.160$ at $d = 10, 20, 30$, against the next best (FastICA: $0.092$, $0.183$, $0.271$)---a reduction of $40$--$45\%$ that holds across all three dimensions. The margin is larger on \textit{Full Hybrid} mixtures (Laplace, Gaussian, Uniform, Student-$t$, Beta combined): OT-ICA achieves $E = 0.046$, $0.211$, $0.308$ against FastICA's $0.255$, $0.451$, $0.671$. On \textit{Zero Gaussian} sources, where logcosh gradients degenerate for one component, OT-ICA yields $E = 0.042$, $0.088$, $0.233$ while InfoMax yields $0.105$, $0.315$, $0.549$.

\textbf{Discrete sources.} On \textit{Discrete Only} mixtures, OT-ICA underperforms JADE at $d = 10$ ($0.643$ vs.\ $0.443$), is competitive at $d = 20$ ($1.231$ vs.\ JADE $1.267$), and all methods saturate the clip ceiling at $d = 30$. Table~\ref{tab:w2_sensitivity} shows $W_2^2$ retains a clear non-Gaussianity signal on count data---exceeding logcosh resolution by more than $10{\times}$ on standard Poisson($\lambda=3$)---so the failure is not a contrast deficiency. Step-function CDFs create gradient plateaus that stall the Riemannian solver despite the contrast being nonzero (Appendix~\ref{app:discrete_contrast}). Gaussian dithering (Appendix~\ref{app:methodology}) is the recommended mitigation for applications with discrete components, partially smoothing the CDF steps, but does not fully restore gradient signal at the scales tested.

\textbf{Method-specific failures.} FastICA and Picard \citep{ablin2018faster} produce nearly identical errors across all configurations (within $0.003$ on continuous regimes at each dimension), establishing that Picard's provably converging orthogonal solver does not close the gap: the bottleneck is the shared tanh nonlinearity, not the optimizer. JADE requires $\mathcal{O}(d^4)$ samples for reliable fourth-order cumulant estimation---approximately $810{,}000$ at $d = 30$ against $N = 10{,}000$ available---yielding $E = 0.489$ on continuous and $0.938$ on hybrid sources at $d = 30$. InfoMax \citep{bell1995information} is misspecified on distributions mixing super- and sub-Gaussian components, producing $E = 0.765$ on hybrid at $d = 30$ against OT-ICA's $0.308$. (Computational scaling in Appendix~\ref{app:scaling}.)

\section{Interdisciplinary Applications}

\subsection{Structural Identification in Price Discovery}
In fragmented financial markets, cointegrated prices share an unobserved common random walk. The Information Share (IS) \citep{hasbrouck1995, baillie2002} quantifies each venue's contribution, requiring decomposition of the reduced-form residuals $u_t$ of a Vector Error Correction Model (VECM) \citep{engle1987, johansen1991} into structural shocks $\epsilon_t$ via $u_t = \mathbf{B}\epsilon_t$.

The covariance constraint $\boldsymbol{\Omega} = \mathbf{B}\mathbf{B}^\top$ leaves $\mathbf{B}$ unidentified up to an orthogonal rotation. Standard Cholesky decomposition resolves this by imposing a recursive market ordering, but IS estimates change with the ordering choice.

Factoring $\mathbf{B} = \mathbf{S}\mathbf{C}$ (whitening times orthogonal rotation) reduces identification to finding $\mathbf{C}$ \citep{zema2025}. OT-ICA recovers $\mathbf{C}$ from the heavy-tailed non-Gaussianity of financial innovations without imposing a recursive ordering.
...
Table~\ref{tab:simulated_is_results} reports mean IS estimates within $0.002$ of the true values across 500 Monte Carlo runs, with standard deviations below $0.03$, without imposing any market ordering.

\subsection{EEG Artifact Removal}
The skull acts as a linear volume conductor \citep{hyvarinen2001}: ocular blink artifacts mix with neural signals before reaching scalp electrodes. Blink artifacts are impulsive and super-Gaussian; their high $W_2^2$ separates them from neural components without specifying a density model.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\columnwidth]{eeg_artifact_removal.pdf}
    \caption{OT-ICA on five frontal EEG channels (MNE sample dataset). Panel (b): isolated blink component (orange) with excess kurtosis $\kappa$ in the panel title; remaining components have mean $\kappa$ near zero. Panel (c): signal reconstructed by zeroing the artifact component, with RMS reduction in the $\pm$250\,ms blink window in the panel title.}
    \label{fig:eeg_artifact}
\end{figure}

Figure~\ref{fig:eeg_artifact} shows OT-ICA concentrating the blink artifact into a single component with excess kurtosis substantially above the remaining four. Zeroing that component and inverting the total unmixing matrix produces the RMS reduction quantified in the panel title, without tuning a density model.

\section{Conclusion}
OT-ICA replaces proxy contrast functions with the exact $W_2^2$ metric computed by quantile sorting, avoiding FastICA's gradient degeneracy, JADE's $\mathcal{O}(d^4)$ sample complexity, and InfoMax's distributional misspecification. The Picard comparison isolates the contrast as the failure mode: a faster solver on the same tanh nonlinearity does not reduce Amari error. Across $d \in \{10, 20, 30\}$ and five mixture regimes ($N=10{,}000$, 10 trials), OT-ICA achieves the lowest Amari error on all continuous and heterogeneous configurations. On purely discrete sources, the contrast is present but gradient plateaus stall the solver---an open algorithmic problem. OT-ICA's sorting-based contrast incurs $\mathcal{O}(K \cdot dN\log N)$ per iteration against FastICA's $\mathcal{O}(dN)$; on purely continuous homogeneous sources, FastICA is faster. This tradeoff reverses at high dimensions on heterogeneous and hybrid mixtures, where proxy-based methods saturate the error ceiling while OT-ICA maintains separation (Appendix~\ref{app:scaling}). Applications to VECM structural identification and EEG artifact removal confirm the method transfers to real data without distributional assumptions. Future work will extend OT-ICA to causal discovery \citep{shimizu2006linear, liang2023causal}.
...
The \textit{Discrete Only} failure is mechanistically distinct from the continuous failure modes above. Table~\ref{tab:w2_sensitivity} compares the non-Gaussianity resolution of $W_2^2$ against the logcosh proxy negentropy $(\mathbb{E}[G(x)]-\mathbb{E}[G(\nu)])^2$ across standard and highly non-Gaussian parameterizations.
...
\begin{figure}[htbp]
    ...
    \caption{$W_2^2$ and logcosh optimization landscapes for two-source Poisson and Binomial mixtures across parameterizations. The $W_2^2$ landscape retains a non-zero signal throughout (confirming the contrast is present), but the step-function geometry of discrete CDFs creates flat plateaus where the gradient evaluates to near-zero, stalling gradient-based solvers. The logcosh landscape degenerates to near-zero for standard count-based parameterizations, indicating a fundamental contrast failure.}
    \label{fig:optim_surfaces}
\end{figure}

\subsection{JADE: Sample Complexity of Fourth-Order Cumulants}
JADE \citep{cardoso1993blind} identifies sources by jointly diagonalizing a set of fourth-order cumulant matrices via Jacobi sweeps. Reliable estimation of the full cumulant tensor requires $\mathcal{O}(d^4)$ samples \citep{cardoso1993blind, hyvarinen2001}. At $d=30$, this implies approximately $810{,}000$ samples; against the $N=10{,}000$ available in our benchmark, the cumulant matrices are heavily undersampled. The resulting joint diagonalization converges on estimated tensors reflecting sampling noise rather than true source structure, yielding $E=0.489$ on continuous sources and $E=0.938$ on hybrid sources at $d=30$---errors that approach or saturate the failure threshold despite JADE's theoretical consistency on well-sampled data.

\subsection{InfoMax: Score Function Misspecification}
InfoMax \citep{bell1995information} maximizes a log-likelihood under a fixed logistic nonlinearity, implicitly treating all sources as sub-Gaussian. On mixtures containing super-Gaussian components (Laplace, Student-$t$), the score function is misspecified: the gradient points away from the true unmixing direction. \citet{lee1999extended} documented this failure mode explicitly, proposing extended InfoMax with online source-type switching. That approach partially addresses the misspecification but requires correct per-component classification---an assumption OT-ICA avoids entirely. In our benchmark, InfoMax produces $E=0.765$ on hybrid sources and $E=0.549$ on Zero Gaussian sources at $d=30$, against OT-ICA's $0.308$ and $0.233$ respectively, consistent with the misspecified-score explanation.

\subsection{Picard: The Contrast as the Binding Constraint}
Picard \citep{ablin2018faster} replaces InfoMax's gradient ascent with an L-BFGS preconditioned solver on the same log-likelihood objective, achieving provably faster convergence per iteration. In our benchmark, FastICA and Picard produce Amari errors within $0.003$ of each other across all continuous configurations at each dimension. This near-identical performance isolates the source of failure: solver speed is not the binding constraint. The shared tanh nonlinearity---and not the optimizer---determines the error floor. Replacing it with an exact, distribution-free contrast reduces error by $40$--$45\%$ on continuous sources.
