...
Following baseline validation, we evaluate the general robustness of OT-ICA against three proxy-based methods across five heterogeneous mixture regimes.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\columnwidth]{general_hybrid_test.pdf}
    \caption{Amari error at $d=30$, $N=10{,}000$ across five mixture regimes. \textit{Discrete Only} is a fundamental limitation shared by all methods (three generator types produce near-indistinguishable sources at $d=30$). On all other regimes OT-ICA achieves the lowest error; FastICA, JADE, and InfoMax degrade most on \textit{Zero Gaussian} and \textit{Full Hybrid} where heterogeneous topologies invalidate their proxy contrasts.}
    \label{fig:hybrid_mixture_results}
\end{figure}

For our primary evaluation we mixed a standard Gaussian with $(d-1)$ sources drawn from five regime pools spanning super-Gaussian, sub-Gaussian, continuous, and discrete distributions (Laplace, Bernoulli, Uniform, Poisson, Exponential). We benchmark four methods: OT-ICA, FastICA \citep{hyvarinen2001}, JADE \citep{cardoso1993blind}, and InfoMax \citep{bell1995information}.

Figure \ref{fig:hybrid_mixture_results} shows that proxy-based methods (FastICA, JADE, InfoMax) degrade on heterogeneous and non-Gaussian topologies, with the steepest failures on \textit{Zero Gaussian} and \textit{Full Hybrid} regimes where conflicting distributional geometries invalidate local contrast surrogates. OT-ICA consistently achieves the lowest Amari error by evaluating the full distributional geometry via $W_2^2$. (Computational scaling is discussed in Appendix~\ref{app:scaling}.)
