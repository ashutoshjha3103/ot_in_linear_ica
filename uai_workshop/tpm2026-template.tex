...
\begin{abstract}
Independent Component Analysis (ICA) is a foundational probabilistic tool for recovering latent, 
statistically independent sources from observed mixtures. Because maximizing marginal non-Gaussianity 
equates to extracting independent components, traditional ICA algorithms achieve tractability by relying on 
local proxy contrast functions (e.g., negentropy approximations) to evaluate these non-Gaussianities. 
However, these proxies systematically fail to converge on complex, heterogeneous topologies. 
We propose OT-ICA, an exploratory framework that evaluates the full probability density by minimizing the 
1D squared Wasserstein distance ($W_2^2$) to a Gaussian target. By exploiting optimal transport theory, 
OT-ICA retains algorithmic efficiency while demonstrating strict robustness on hybrid mixtures where traditional 
fixed-point solvers fail. Finally, we demonstrate the utility of this framework in two interdisciplinary applications: 
isolating ocular artifacts in clinical EEG recordings, and identifying structural market shocks to resolve the arbitrary 
recursive ordering (Cholesky) problem in standard Vector Error Correction Models (VECMs).
\end{abstract}
...
Prior work uses 1D $W_2$ for normality testing \citep{delbarrio1999} or marginal distribution comparison \citep{peyre2019computational}, but none formalizes $W_2^2$-to-Gaussian as an ICA contrast.
...
Efficient optimization of $W_2^2$ on the Space of Orthogonal Matrices or the Othogonal Group requires three enhancements: (i)~\textbf{Analytical Gaussian Targets} \citep{fournier2015rate} to eliminate target-sampling noise; (ii)~\textbf{Riemannian Gradient Retraction} onto the Orthogonal Group \citep{absil2008optimization}; and (iii)~\textbf{Gaussian Dithering} to smooth discrete-CDF plateaus \citep{schuchman1964dither}. Full formulations and pseudocode are in Appendix~\ref{app:methodology}.
...
We benchmark OT-ICA, FastICA \citep{hyvarinen2001}, JADE \citep{cardoso1993blind}, and InfoMax \citep{bell1995information} across five mixture regimes at $d=30$, $N=10{,}000$.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\columnwidth]{general_hybrid_test.pdf}
    \caption{Amari error across five mixture regimes ($d=30$, $N=10{,}000$). OT-ICA achieves the lowest error on all continuous regimes. On \textit{Discrete Only} all methods fail — see Appendix~\ref{app:proxy_limits} for the contrast vs.\ optimization analysis.}
    \label{fig:hybrid_mixture_results}
\end{figure}

Figure~\ref{fig:hybrid_mixture_results} confirms three structurally distinct proxy failures. \textbf{FastICA} fails via (i)~\textit{zero negentropy}: specific non-Gaussian sources satisfy $\mathbb{E}[G(x)]=\mathbb{E}[G(\nu)]$, zeroing the contrast gradient; and (ii)~\textit{vanishing curvature}: the Newton-update denominator $\mathbb{E}[s g(s) - g'(s)] \to 0$, rendering the fixed-point step undefined (Appendix~\ref{app:proxy_limits}). \textbf{JADE} requires $\mathcal{O}(d^4)$ samples for reliable fourth-order cumulant estimation \citep{cardoso1993blind} — ${\approx}810{,}000$ at $d=30$ versus $N=10{,}000$ available — so its joint diagonalization is dominated by noise. \textbf{InfoMax}'s parametric log-likelihood model \citep{bell1995information} breaks on topologies that mix super- and sub-Gaussian sources simultaneously. OT-ICA succeeds in all continuous and heterogeneous regimes by evaluating the full distributional geometry via $W_2^2$ without any parametric or curvature assumptions.

The \textit{Discrete Only} failure is mechanistically distinct: a \textit{contrast} failure for FastICA but an \textit{optimization} bottleneck for OT-ICA. Table~\ref{tab:w2_sensitivity} (Appendix~\ref{app:proxy_limits}) shows $W_2^2$ provides ${\gg}10\times$ the non-Gaussianity resolution of logcosh for count-based data — standard Poisson ($\lambda=3$) has near-zero logcosh negentropy yet a clear $W_2^2$ signal of $0.05$. OT-ICA still detects the non-Gaussianity; it is the step-function CDFs of discrete data that create flat gradient plateaus stalling the solver. (Scaling in Appendix~\ref{app:scaling}.)
...
\begin{figure}[t]
    ...
    \caption{OT-ICA applied to five frontal EEG channels (MNE sample dataset). The high-amplitude synchronized blink artifact is isolated into a single super-Gaussian independent component (Comp 1, orange), leaving the underlying brain-wave structure intact in the remaining components.}
...
\section{Conclusion}
By formulating ICA as a tractable 1D optimal transport problem, OT-ICA overcomes geometric limitations of proxy contrast solvers — including FastICA's gradient degeneracy, JADE's sample complexity, and InfoMax's model mismatch — and remains robust on the heterogeneous and zero-Gaussian regimes where all three fail. Applied to VECM price discovery and clinical EEG, it demonstrates practical utility beyond the benchmark. Future work will extend OT-ICA to causal discovery frameworks including LiNGAM \citep{shimizu2006linear} and CauCA \citep{liang2023causal}.
