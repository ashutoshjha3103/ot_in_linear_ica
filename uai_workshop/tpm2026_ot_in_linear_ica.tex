...
\begin{abstract}
Linear Independent Component Analysis (ICA) recovers mutually independent sources from linear mixtures.
Classical algorithms like FastICA, JADE, and InfoMax compute non-Gaussianity via surrogate contrast functions like negentropy approximations,
fourth-order cumulants, parametric log-likelihoods, that fail on heterogeneous and high-dimensional mixtures.
We propose Optimal Transport metric for ICA, OT-ICA, using the 1D squared Wasserstein distance ($W_2^2$) to a standard Gaussian as contrast,
...
Across $d \in \{10, 20, 30\}$ and five mixture regimes ($N = 10{,}000$),
OT-ICA outperforms FastICA, JADE, InfoMax, and Picard on all continuous and heterogeneous distributions.
On purely discrete sources, while $W_2^2$ retains high contrast resolution, step-function CDFs produce gradient plateaus that hinder convergence.
Two applications are presented: EEG ocular artifact removal and structural shock identification in Vector Error Correction Models for financial price discovery.
\end{abstract}
...
We propose OT-ICA, using squared Wasserstein metric, $W_2^2$ to a standard Gaussian as the contrast. Unlike proxy contrasts, Optimal Transport \citep{monge1781, kantorovich1942}
provides a true metric on the space of probability distributions,
quantifying non-Gaussianity as the cost of transporting the empirical projection to a Gaussian target. The $p$-Wasserstein distance \citep{villani2003} is the minimum-cost coupling:
\begin{equation}
...
Three components are required for optimization on the Orthogonal Group $\mathcal{S}$: (i)~\textbf{Analytical Gaussian Targets} \citep{fournier2015rate}, 
...
confirming that the Riemannian solver with analytical Gaussian targets converges reliably on continuous distributions. On this 4D mixture, 
OT-ICA achieves an Amari error of $0.016$ against FastICA's $0.019$; with it's global transfer matrix within $10^{-2}$ 
of a generalized permutation matrix. (Table~\ref{tab:baseline_matrices}, Appendix~\ref{app:experimental_setup}).
...
OT-ICA recovers $\mathbf{C}$ from the heavy-tailed non-Gaussianity of financial innovations without imposing a recursive ordering (DGP, IS definition, and estimation procedure in Appendix~\ref{app:price_discovery}).
...
\multirow{3}{*}{Pure Student-$t$}
 ...
 & 30 & --- & --- & --- & --- & --- \\
\midrule
\multirow{3}{*}{Pure Bernoulli}
 & 10 & --- & --- & --- & --- & --- \\
 & 20 & --- & --- & --- & --- & --- \\
 & 30 & --- & --- & --- & --- & --- \\
\midrule
\multirow{3}{*}{Pure Poisson}
 & 10 & --- & --- & --- & --- & --- \\
 & 20 & --- & --- & --- & --- & --- \\
 & 30 & --- & --- & --- & --- & --- \\
\midrule
\multirow{3}{*}{Pure Binomial}
 & 10 & --- & --- & --- & --- & --- \\
 & 20 & --- & --- & --- & --- & --- \\
 & 30 & --- & --- & --- & --- & --- \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Mean Amari error ($\downarrow$ better, clipped at 1.500) for all five methods and mixture regimes ($N=10{,}000$, 10 trials). OT-ICA achieves the lowest error on all continuous and heterogeneous configurations. On Discrete Only, JADE leads at $d=10$; OT-ICA leads at $d=20$ and is the only method below the clip ceiling at $d=30$. Pure single-distribution rows filled after running \texttt{tpm2026\_figures.py} Cell~8b.}
