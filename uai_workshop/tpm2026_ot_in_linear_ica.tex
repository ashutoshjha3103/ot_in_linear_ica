...
\begin{figure*}[!b]
    ...
    (Table~\ref{tab:w2_sensitivity}, Appendix~\ref{app:discrete_contrast}). At $d=30$, all proxy-based methods saturate the clip ceiling on discrete and several heterogeneous regimes; OT-ICA is the only method below the ceiling on Discrete Only ($E=1.486$). Dotted line at $E=0.3$ marks the good-separation threshold.}
...
heterogeneous configurations; on purely discrete sources, gradient plateaus stall the solver, an open optimization problem.
OT-ICA incurs higher per-iteration cost than FastICA at large $d$ (Appendix~\ref{app:scaling}), but this tradeoff reverses on heterogeneous mixtures where proxy-based methods saturate the error ceiling.
