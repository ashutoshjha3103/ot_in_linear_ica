...
\author[1,3]{\href{mailto:ashutosh.jha@student.uni-tuebingen.de?Subject=TPM 2026 Paper Inquiry}{Ashutosh~Jha}}
...
We propose instead to measure non-Gaussianity using the squared Wasserstein distance $W_2^2$ to a standard Gaussian. %Since $W_2^2$ is a proper metric on probability distributions, this measure only vanishes for the Gaussian distribution.
...
Picard \citep{ablin2018faster} an adaptive nonlinearity with a provably converging solver. These proxies have shortcomings such as evaluating to zero on   specific non-Gaussian source distributions (Appendix~\ref{app:proxy_limits}).
...
maximizes this contrast over the direction $\mathbf{b}$
\begin{equation}
...
We find the following immediate corollary.
\begin{corollary}
...
%\subsection{Optimization on Orthogonal Group}
After whitening the data, the projection directions associated to all independent components are mutually orthogonal, which restricts the search: each source direction must lie in the orthogonal complement of the already-found directions.
One can therefore exploit OT-ICA for a single component iteratively, extracting one component at a time from the shrinking orthogonal complement.
In practice we implement this as symmetric joint optimization \citep{hyvarinen2001}: all $d$ rows of $\mathbf{B}$ are optimized simultaneously and the $\mathrm{O}(d)$ constraint is restored via symmetric decorrelation retraction at every step. 
The full details of the resulting algorithm can be found in Appendix~\ref{app:methodology} and pseudocode is given in Algorithm~\ref{alg:ot_ica}.
Let us here just briefly mention
three important algorithmic components: (i)~\textbf{Analytical Gaussian Targets} \citep{fournier2015rate}, replacing sampled quantile targets of the standard normal distribution with closed-form bin expectations to eliminate approximation noise; (ii)~\textbf{Riemannian Gradient Retraction} \citep{absil2008optimization}, projecting gradients onto the tangent space of Orthogonal Group $\mathrm{O}(d)$ and retracting via symmetric decorrelation;% (Newton updates are intractable for empirical OT; Appendix~\ref{app:intractability}); 
and (iii)~\textbf{Gaussian Dithering} \citep{schuchman1964dither}, smoothing discrete CDFs via narrow Gaussian convolution.
