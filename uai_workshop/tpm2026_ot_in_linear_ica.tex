...
We propose OT-ICA, which measures non-Gaussianity using the squared Wasserstein distance $W_2^2$ to a standard Gaussian. 
We prove that the Wasserstein distance of linear projections of the data to a standard normal distribution is maximized when an independent component is recovered.
$W_2^2$ of any proper mixture of non-Gaussian sources lies strictly below the weighted sum of the individual component $W_2^2$ values, establishing a unique global maximum at pure source directions and ensuring the ICA optimization is well-posed.
...
We now study the empirical perfomance of the proposed algorithm\footnote{Code to reproduce the experiments is available under \url{https://anonymous.4open.science/r/ot_in_linear_ica-3DBC}}
