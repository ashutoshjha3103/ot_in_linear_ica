...
\begin{abstract}
Independent Component Analysis (ICA) recovers mutually independent sources from linear mixtures. 
Classical algorithms like FastICA, JADE, and InfoMax compute non-Gaussianity via surrogate contrast functions e.g. negentropy approximations, 
fourth-order cumulants, parametric log-likelihoods, that fail on heterogeneous source distributions. 
We propose OT-ICA, which uses the 1D squared Wasserstein distance ($W_2^2$) to a standard Gaussian as the contrast function, 
computed by quantile sorting without density estimation. 
We prove $W_2^2$ satisfies a weighted mixture upper bound with strict inequality for non-Gaussian sources, establishing it as a valid ICA contrast. 
Across $d \in \{10, 20, 30\}$ and five mixture regimes at sample size ($N = 10{,}000$, 10 trials), 
OT-ICA achieves better independent component extraction scores than FastICA, JADE, InfoMax, and Picard on all continuous and heterogeneous distributions. 
On purely discrete sources, while the contrast via Wasserstein metric is present, the step-function CDFs produce gradient plateaus that stall the OT-ICA solver. 
Two applications are presented: ocular artifact removal from frontal EEG, and structural shock identification in Vector Error Correction Models for financial price discovery.
\end{abstract}
...
Assume (i) $\mathbf{X} = \mathbf{A}\mathbf{S}$ with $\mathbf{A}$ invertible, (ii) $s_1, \dots, s_d$ mutually independent, 
and (iii) at most one $s_i$ Gaussian. Then $\mathbf{A}$ is identifiable up to column permutation and scaling.
\end{theorem}

The Central Limit Theorem implies that linear mixtures of independent non-Gaussian sources are more Gaussian than their constituents. 
ICA recovers sources by maximizing marginal non-Gaussianity over the orthogonal group \citep{hyvarinen2001}, justified by the following identity.

\begin{theorem}[Independence and Non-Gaussianity \citep{cardoso2022}]
Let $Y$ be whitened ($\operatorname{Cov}(Y)=\mathbf{I}$). The mutual information satisfies:
\begin{equation}
    \mathscr{I}(Y) = G(Y) - \sum_{i} G(Y_i)
\end{equation}
where $G(\cdot)$ denotes KL divergence to the best-fitting Gaussian. Since $G(Y)$ is invariant under orthogonal rotation, 
minimizing $\mathscr{I}(Y)$ is equivalent to maximizing $\sum_i G(Y_i)$.
\end{theorem}
\textit{(Proof via KL Pythagorean identities on the Product and Gaussian manifolds: Appendix~\ref{app:info_geometry}.)}

Evaluating $G(Y_i)$ requires the marginal density. Classical algorithms replace it with surrogates: 
...
We propose OT-ICA, using $W_2^2$ to a standard Gaussian as the contrast. Unlike proxy contrasts, OT provides a true metric on the space of probability distributions, quantifying non-Gaussianity as the cost of transporting the empirical projection to a Gaussian target \citep{villani2003}. The Kantorovich relaxation \citep{monge1781, kantorovich1942} defines the $p$-Wasserstein distance as the minimum-cost coupling \citep{villani2003}:
\begin{equation}
...
distribution comparison \citep{peyre2019computational}; we formalize $W_2^2$-to-Gaussian as a valid ICA contrast with finite-sample convergence guarantees.

\section{The OT-ICA Framework}

After centering and whitening $\mathbf{X}$ to $\widetilde{\mathbf{X}}$, 
the unmixing matrix is constrained to the Orthogonal Group $\mathcal{S}$ \citep{cardoso2022}. 
...
The objective is maximized at a pure source direction, established by two bounds.

\begin{theorem}[$W_2$ Upper Bound]
Let $\mathbf{Z} = (Z_1, \dots, Z_d)^\top$ be a vector of mutually independent latent sources, 
and let $\mathbf{b}$ be a weight vector on the unit sphere ($\sum_{i=1}^d b_i^2 = 1$). 
The squared Wasserstein distance between the linear mixture $\mathbf{b} \cdot \mathbf{Z}$ and 
the standard normal distribution $\Gamma$ is bounded by the weighted sum of the source distances to $\Gamma$:
\begin{equation}
...
Assuming at most one source $Z_i$ is Gaussian, and the source distributions possess smooth and strictly positive densities, 
the upper bound derived in Theorem 3 is a strict inequality for any valid mixture where multiple weights $b_i$ are non-zero:
\begin{equation}
    W_2^2(\mathbf{b} \cdot \mathbf{Z}, \Gamma) < \sum_{i=1}^d b_i^2 W_2^2(Z_i, \Gamma).
\end{equation}
\end{theorem}

The global maximum is attained when $\mathbf{b}$ aligns with a single source direction. 
Proofs via comonotonicity contradictions under Caffarelli's regularity theory are in Appendix~\ref{app:proofs}.

\subsection{Optimization on Orthogonal Group $\mathcal{S}$}
Three components are required for optimization on $\mathcal{S}$: (i)~\textbf{Analytical Gaussian Targets} \citep{fournier2015rate}, 
replacing sampled quantile targets with their closed-form bin expectations to eliminate approximation noise; 
(ii)~\textbf{Riemannian Gradient Retraction} \citep{absil2008optimization}, projecting gradients onto the tangent space 
of $\mathcal{S}$ and retracting via symmetric decorrelation; and 
(iii)~\textbf{Gaussian Dithering} \citep{schuchman1964dither}, convolving discrete empirical distributions 
with a narrow Gaussian kernel to smooth step-function CDFs. Full formulations and pseudocode are in Appendix~\ref{app:methodology}.
...
generalized permutation matrix ($E < 0.1$: near-perfect; $E \ge 0.5$: failure; definition in Appendix~\ref{app:experimental_setup}).
...
Figure~\ref{fig:ot_ica_validation} shows $W_2^2$ driving the Amari error to near zero on Laplace, Uniform, Student-$t$, and Beta sources, confirming that the Riemannian solver with analytical Gaussian targets converges reliably on continuous distributions. On this 4D mixture, OT-ICA achieves an Amari error of $0.016$ against FastICA's $0.019$; both global transfer matrices are within $10^{-2}$ of a generalized permutation matrix (Table~\ref{tab:baseline_matrices}, Appendix~\ref{app:experimental_setup}).
...
\begin{figure*}[t]
    ...
    OT-ICA achieves the lowest error on all continuous and heterogeneous regimes at every dimension.
    On \textit{Discrete Only}, the $W_2^2$ contrast is present but step-function CDFs produce gradient plateaus that stall the solver
    (Table~\ref{tab:w2_sensitivity}, Appendix~\ref{app:discrete_contrast}). At $d=30$, all proxy-based methods saturate the clip ceiling on discrete and several heterogeneous regimes; OT-ICA is the only method below the ceiling on Discrete Only ($E=1.486$).}
...
\textbf{Continuous and heterogeneous regimes.} OT-ICA achieves the lowest Amari error on all four continuous and mixed-type configurations at every dimension (Table~\ref{tab:full_amari_results}, Appendix~\ref{app:experimental_setup}). On \textit{Continuous Only} sources, OT-ICA reduces error $40$--$45\%$ over FastICA across all three dimensions. The margin is larger on \textit{Full Hybrid} mixtures (Laplace, Gaussian, Uniform, Student-$t$, Beta combined): OT-ICA attains $E = 0.046$, $0.211$, $0.308$ at $d = 10, 20, 30$ against FastICA's $0.255$, $0.451$, $0.671$---a factor of $2$--$5\times$. On \textit{Zero Gaussian} sources, OT-ICA outperforms InfoMax, which is misspecified for that mixture type, by a similar margin at all dimensions.

\textbf{Discrete sources.} On \textit{Discrete Only} mixtures, OT-ICA underperforms JADE at $d = 10$ ($0.643$ vs.\ $0.443$), 
is competitive at $d = 20$ ($1.231$ vs.\ JADE $1.267$), and at $d = 30$ all proxy-based methods saturate the clip ceiling ($1.500$) while OT-ICA reaches $1.486$. 
Table~\ref{tab:w2_sensitivity} shows $W_2^2$ retains a clear non-Gaussianity signal on count data, exceeding logcosh resolution 
by more than $10{\times}$ on standard Poisson($\lambda=3$), so the failure is not a contrast deficiency. 
Step-function CDFs create gradient plateaus that stall the Riemannian solver despite the contrast being non-zero (Appendix~\ref{app:discrete_contrast}). 
Gaussian dithering (Appendix~\ref{app:methodology}) is the technique used for avoiding optimization surface discontinuities 
with discrete components, partially smoothing the CDF steps, but does not fully restore gradient signal at the scales tested.

\textbf{Method-specific failures.} FastICA and Picard \citep{ablin2018faster} produce nearly identical errors across all configurations 
(within $0.003$ on continuous regimes at each dimension), establishing that Picard's provably converging orthogonal solver does not close the gap: 
the bottleneck is the shared tanh nonlinearity, not the optimizer. JADE requires $\mathcal{O}(d^4)$ samples for reliable fourth-order cumulant estimation, 
approximately $810{,}000$ at $d = 30$ against $N = 10{,}000$ available, yielding $E = 0.489$ on continuous and $0.938$ on hybrid sources at $d = 30$. 
InfoMax \citep{bell1995information} is misspecified on distributions mixing super- and sub-Gaussian components, 
producing $E = 0.765$ on hybrid at $d = 30$ against OT-ICA's $0.308$. (Computational scaling in Appendix~\ref{app:scaling}.)

\section{Interdisciplinary Applications}

\subsection{Structural Identification in Price Discovery}
In fragmented financial markets, cointegrated prices share an unobserved common random walk. 
The Information Share (IS) \citep{hasbrouck1995, baillie2002} quantifies each venue's contribution, 
requiring decomposition of the reduced-form residuals $u_t$ of a Vector Error Correction Model (VECM) \citep{engle1987, johansen1991} 
into structural shocks $\epsilon_t$ via $u_t = \mathbf{B}\epsilon_t$.

The covariance constraint $\boldsymbol{\Omega} = \mathbf{B}\mathbf{B}^\top$ leaves $\mathbf{B}$ unidentified up to an orthogonal rotation. 
Standard Cholesky decomposition resolves this by imposing a recursive market ordering, but IS estimates change with the ordering choice.

Factoring $\mathbf{B} = \mathbf{S}\mathbf{C}$ (whitening times orthogonal rotation) reduces identification to finding $\mathbf{C}$ \citep{zema2025}. 
OT-ICA recovers $\mathbf{C}$ from the heavy-tailed non-Gaussianity of financial innovations without imposing a recursive ordering.
...
\begin{table}[htbp]
    ...
    \caption{Simulation Results for Information Share Recovery across 500 Monte Carlo runs. 
    The estimated IS tightly matches the true IS without imposing arbitrary Cholesky recursive structures.}
    \label{tab:simulated_is_results}
\end{table}

Table~\ref{tab:simulated_is_results} reports mean IS estimates within $0.002$ of the true values across 500 Monte Carlo runs, 
with standard deviations below $0.03$, without imposing any market ordering.
...
Blink artifacts are impulsive and super-Gaussian; their high $W_2^2$ separates them from neural components without specifying a density model.
...
\section{Conclusion}
OT-ICA replaces proxy contrast functions with the exact $W_2^2$ metric computed by quantile sorting, 
avoiding FastICA's gradient degeneracy, JADE's $\mathcal{O}(d^4)$ sample complexity, and InfoMax's distributional misspecification. 
The Picard comparison isolates the contrast as the failure mode: a faster solver on the same tanh nonlinearity does not reduce Amari error. 
Across $d \in \{10, 20, 30\}$ and five mixture regimes ($N=10{,}000$, 10 trials), OT-ICA achieves the lowest Amari error on all continuous and 
heterogeneous configurations. On purely discrete sources, the contrast is present but gradient plateaus stall the solver, 
an open optimization problem. OT-ICA's sorting-based contrast incurs $\mathcal{O}(K \cdot dN\log N)$ per iteration against 
FastICA's $\mathcal{O}(dN)$; on purely continuous homogeneous sources, FastICA is faster. This tradeoff reverses at high dimensions 
on heterogeneous and hybrid mixtures, where proxy-based methods saturate the error ceiling while OT-ICA maintains separation (Appendix~\ref{app:scaling}). 
Applications to VECM structural identification and EEG artifact removal confirm the method transfers to real data without distributional assumptions. 
Future work will extend OT-ICA to causal discovery, including as a front-end for LiNGAM \citep{shimizu2006linear}, Causal Component Analysis \citep{liang2023causal}, and identifiable nonlinear ICA \citep{buchholz2022function}.
...
Information geometry provides a framework for quantifying the relationship between independence and non-Gaussianity by analyzing 
probability distributions as points on geometric manifolds. Following the perspective introduced by \citet{cardoso2022}, 
ICA can be understood as orthogonally projecting the empirical distribution $P_Y$ onto two distinct subspaces: 
the Product manifold $\mathcal{P}$ (containing distributions where marginals are independent) and the Gaussian manifold $\mathcal{G}$ (encompassing all multivariate Gaussians). 
...
The Mutual Information $\mathscr{I}(Y)$ measures the distance from $P_Y$ to its closest independent product distribution $P_Y^P$. 
...
A similar Pythagorean identity holds for the Gaussian manifold $\mathcal{G}$. Let $\Phi_Y = \mathcal{N}(0, \operatorname{Cov} Y)$ be 
the best Gaussian fit (sharing the exact mean and covariance of $P_Y$), and let $\Phi = \mathcal{N}(0, \Sigma)$ be an arbitrary Gaussian target. 
The divergence decomposes as:
\begin{align}
...
Because the log-density of a zero-mean Gaussian is a quadratic form, its expectation depends only on the second moments 
(the covariance matrix) of the variable. Since the true distribution $P_Y$ and its Gaussian fit $\Phi_Y$ share the exact same covariance matrix by definition, 
evaluating the expectation of this quadratic form under $P_Y$ is identical to evaluating it under $\Phi_Y$:
\begin{equation}
...
We define the joint non-Gaussianity as $G(Y) = D_{\mathrm{KL}}(P_Y \,\Vert\, \mathcal{N}(\operatorname{Cov} Y))$, 
and correlation as $C(Y) = D_{\mathrm{KL}}(\mathcal{N}(\operatorname{Cov} Y) \,\Vert\, \mathcal{N}(\operatorname{diag\,Cov} Y))$. 
By evaluating the KL divergence from $P_Y$ to a fully factorized Gaussian target $\mathcal{N}(\operatorname{diag\,Cov} Y)$ via both 
the Product path and Gaussian path, we yield Cardoso's unified theorem \citep{cardoso2022}:
\begin{equation}
    \mathscr{I}(Y) + \sum_{i} G(Y_i) = G(Y) + C(Y).
\end{equation}

Crucially, once the data is whitened ($\operatorname{Cov}(Y) = \mathbf{I}$), correlation drops to zero ($C(Y) = 0$). The relationship rearranges as:
\begin{equation}
    \mathscr{I}(Y) = G(Y) - \sum_{i} G(Y_i).
\end{equation}
Because joint non-Gaussianity $G(Y)$ is invariant under orthogonal rotation, minimizing mutual information (achieving independence) 
equates to maximizing the sum of marginal non-Gaussianities $\sum G(Y_i)$.
...
\begin{theorem}[$W_2$ Upper Bound]
Let $\mathbf{Z} = (Z_1, \dots, Z_d)^\top$ be a vector of mutually independent latent sources, and let $\mathbf{b}$ be a 
weight vector on the unit sphere ($\sum_{i=1}^d b_i^2 = 1$). The squared Wasserstein distance between the linear mixture $\mathbf{b} \cdot \mathbf{Z}$ and 
the standard normal distribution $\Gamma$ is bounded by the weighted sum of the source distances to $\Gamma$:
\begin{equation}
    W_2^2(\mathbf{b} \cdot \mathbf{Z}, \Gamma) \le \sum_{i=1}^d b_i^2 W_2^2(Z_i, \Gamma).
\end{equation}
...
\begin{figure}[htbp]
    ...
    \caption{Gradient Landscapes of $W_1$ and $W_2^2$ across continuous rotation angles $\theta$. The $W_2^2$ gradient smoothly decelerates toward zero, 
    whereas the $W_1$ gradient exhibits jagged volatility on finite empirical samples.}
    \label{fig:w1_vs_w2_derivatives}
\end{figure}

As shown in Figure \ref{fig:w1_vs_w2_derivatives}, the $W_1$ cost lacks the convexity needed to smooth out empirical noise. Its derivative fluctuates frequently, 
causing first-order gradient ascent solvers to stall in local non-smooth traps. The squared penalty in the $W_2^2$ metric acts as a structural smoother, 
penalizing local distortions and providing a continuous, decelerating gradient profile that allows for reliable convergence.
...
\begin{figure}[htbp]
    ...
    \caption{Geometric sketch of optimization on the Orthogonal Group $\mathcal{S}$. The Euclidean gradient $\mathbf{G}$ is projected onto 
...
Table~\ref{tab:full_amari_results} reports mean Amari errors for all five methods across all mixture regimes at $d \in \{10, 20, 30\}$ ($N=10{,}000$, 10 trials). Values at 1.500 indicate total separation failure (clip ceiling). \textbf{Bold} marks the lowest error per row.
...
\multirow{3}{*}{Continuous Only}
 & 10 & \textbf{0.051} & 0.092 & 0.146 & 0.100 & 0.093 \\
 & 20 & \textbf{0.107} & 0.183 & 0.321 & 0.188 & 0.184 \\
 & 30 & \textbf{0.160} & 0.271 & 0.489 & 0.277 & 0.271 \\
\midrule
\multirow{3}{*}{Strictly Super-Gaussian}
 & 10 & \textbf{0.058} & 0.089 & 0.146 & 0.103 & 0.090 \\
...
\multirow{3}{*}{Zero Gaussian}
 ...
 & 20 & \textbf{0.088} & 0.327 & 0.366 & 0.315 & 0.403 \\
 & 30 & \textbf{0.233} & 0.510 & 0.686 & 0.549 & 0.628 \\
\midrule
\multirow{3}{*}{Full Hybrid}
 & 10 & \textbf{0.046} & 0.255 & 0.216 & 0.297 & 0.277 \\
 & 20 & \textbf{0.211} & 0.451 & 0.571 & 0.558 & 0.555 \\
 & 30 & \textbf{0.308} & 0.671 & 0.938 & 0.765 & 0.917 \\
\midrule
\multirow{3}{*}{Discrete Only}
 & 10 & 0.643 & 0.613 & \textbf{0.443} & 0.757 & 0.835 \\
 & 20 & \textbf{1.231} & 1.402 & 1.267 & 1.495 & 1.440 \\
 & 30 & \textbf{1.486} & 1.500 & 1.500 & 1.500 & 1.500 \\
\bottomrule
\end{tabular}
}
\caption{Mean Amari error ($\downarrow$ better, clipped at 1.500) for all five methods and mixture regimes ($N=10{,}000$, 10 trials). OT-ICA achieves the lowest error on all continuous and heterogeneous configurations. On Discrete Only, JADE leads at $d=10$; OT-ICA leads at $d=20$ and is the only method below the clip ceiling at $d=30$.}
...
where $\nu \sim \mathcal{N}(0,1)$. If a latent independent source possesses a non-Gaussian distribution such that its expected value under 
the contrast function equals that of a Gaussian, the approximated negentropy evaluates to zero. The objective function provides no gradient signal, 
and the algorithm fails to extract the source.

\subsection{The Vanishing Curvature Condition}
FastICA utilizes a Newton (second-order) fixed-point iteration. Defining $g(x) = G'(x)$ and $g'(x) = G''(x)$, FastICA approximates the Hessian matrix dynamically. 
For a given component $i$, the diagonal entry scales according to the expectation $\mathbf{H}_{ii} \approx \mathbb{E}\big[s_i g(s_i) - g'(s_i)\big]$.

The Newton update requires applying the inverse of this Hessian ($\mathbf{H}^{-1}$). If a non-Gaussian source distribution causes the expectation in 
the denominator to evaluate to zero, the inverse becomes undefined. This analytical divide-by-zero error makes the update step intractable.
...
on continuous sources and $E=0.938$ on hybrid sources at $d=30$---errors that approach or saturate the failure threshold despite JADE's theoretical consistency on well-sampled data.
...
In our benchmark, InfoMax produces $E=0.765$ on hybrid sources and $E=0.549$ on Zero Gaussian sources at $d=30$, against OT-ICA's $0.308$ and $0.233$ respectively, 
...
\begin{figure}[htbp]
    ...
    the same curse-of-dimensionality noise ceiling at $d=40$---an artifact of finite-sample sparsity shared by all empirical ICA methods at this regime, not a deficiency specific to OT-ICA.}
\end{figure}
