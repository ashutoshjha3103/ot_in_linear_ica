...
We propose to measure non-Gaussianity using the squared Wasserstein distance $W_2^2$ to a standard Gaussian. Since $W_2^2$ is a proper metric on probability distributions, this measure only vanishes for the Gaussian distribution.
...
Algorithms for learning $\mathbf{A}$ from data generally rely on the observation that linear mixtures of independent non-Gaussian sources are more Gaussian than their constituents \citep{hyvarinen2001}, indeed in the limit of many sources convergence to a Gaussian follows from the central limit theorem.
A rigorous statment of this observation was given in \citet{cardoso2022}.
...
Picard \citep{ablin2018faster} an adaptive nonlinearity with a provably converging solver. These proxies have shortcomings such as vanishing on   specific non-Gaussian source distributions (Appendix~\ref{app:proxy_limits}).

In this paper we use Optimal Transport (OT) \citep{monge1781, kantorovich1942}
to define a contrast.
This allows us to apply OT to linear ICA and extends the long list of applications of optimal transport techniques in machine learning such as normality testing \citep{delbarrio1999} and
distribution comparison \citep{peyre2019computational}.
More concretely, we propose to quantify non-Gaussianity as the cost of transporting the (empirical) projection of the observations to a fixed standard Gaussian reference.
The $L_p$-Wasserstein distance \citep{villani2003} is the minimum-cost coupling over joint distributions (Appendix~\ref{app:info_geometry}):
\begin{equation}
...
\end{equation}
Since $W_2^2$ defines a true metric on probability distributions, this contrast function is non-zero on all non-Gaussian distributions.
Theorem~\ref{th:main} below shows the contrast is maximized at independent components, motivating OT-ICA: gradient ascent on $W_2^2$ over $\mathrm{O}(d)$.

The rest of this paper is structured as: Section~\ref{sec:OT-framework} formalizes the framework; Sections~\ref{sec:validation}--\ref{sec:empirical} benchmark it empirically; Section~\ref{sec:applications} demonstrates real-data applications.

\section{The OT-ICA Framework}\label{sec:OT-framework}

We consider the standard linear ICA setting $\Xb = \Ab\Sb$. Centering and whitening $\mathbf{X}$ to $\widetilde{\mathbf{X}}$ (i.e., $\mathbb{E}[\widetilde{\mathbf{X}}] = \mathbf{0}$, $\operatorname{Cov}[\widetilde{\mathbf{X}}] = \mathbf{I}$) constrains the unmixing matrix to the Orthogonal Group $\mathrm{O}(d)$ (the set of $d\times d$ real matrices satisfying $\mathbf{B}\mathbf{B}^\top = \mathbf{I}$); this preprocessing is lossless for identifiable ICA \citep{hyvarinen2001, cardoso2022} (Appendix~\ref{app:info_geometry}).
OT-ICA finds the projection $\mathbf{b}$ maximizing $W_2^2$ to the standard Gaussian $\Gamma \equiv \mathcal{N}(0,1)$:
\begin{equation}
    \hat{\mathbf{b}} = \arg\max_{\mathbf{b} \in \mathrm{O}(d)} W_2^2\big(\mathbb{P}_{\mathbf{b}^\top \widetilde{\mathbf{X}}}, \Gamma\big). \label{eq:otica_obj}
\end{equation}

The following theorem ensures the optimization in \eqref{eq:otica_obj} is well-posed: any proper mixture of non-Gaussian sources yields a $W_2^2$ value below that of its most non-Gaussian component, so the maximum is uniquely attained at pure source directions.
...
\section{OT-ICA Method Validation}\label{sec:validation}
...
OT-ICA achieves $E = 0.016$ against FastICA's $0.019$ on this 4D mixture (Table~\ref{tab:baseline_matrices}, Appendix~\ref{app:experimental_setup}).
...
We now study the empirical performance of the proposed algorithm.\footnote{Code to reproduce the experiments is available under \url{https://anonymous.4open.science/r/ot_in_linear_ica-3DBC}}
...
\section{Interdisciplinary Applications}\label{sec:applications}
...
OT-ICA replaces proxy contrast functions with the true $W_2^2$ metric computed by quantile sorting, outperforming proxy-based baselines on all continuous and heterogeneous distributions tested; the Picard comparison confirms the contrast, not the solver, as the binding constraint. Applications to VECM structural identification and EEG artifact removal confirm transfer to real data without distributional assumptions. On purely discrete sources, all ICA methods fail as $d$ increases ($E > 0.3$); for OT-ICA the $W_2^2$ contrast retains signal throughout but step-function CDFs stall the solver, and per-iteration cost grows with $d$ (Appendix~\ref{app:scaling}). These limitations point to concrete improvements: Sinkhorn distances \citep{cuturi2013sinkhorn, peyre2019computational} as a differentiable substitute for Gaussian dithering, and amortized optimal transport to reduce per-inference sorting cost. Beyond these, OT-ICA's distribution-free contrast opens new directions in causal discovery, including extensions to LiNGAM \citep{shimizu2006linear}, Causal Component Analysis \citep{liang2023causal}, and identifiable nonlinear ICA \citep{buchholz2022function}.
