...
it evaluates the true global geometry of the empirical distribution using order statistics (sorted data), providing a non-parametric measure of distance to Gaussianity. 
...
Chapter 5 presents our experimental evaluation, demonstrating the failure modes of standard proxy optimization (such as FastICA) and how OT-ICA addresses these limitations. 
...
\end{equation}
where $\mathbf{A} \in \mathbb{R}^{d \times d}$ is an unknown, invertible mixing matrix \cite[Chapter 7]{hyvarinen2001}. Expanded into vector notation, the model represents the observed variables as
\begin{equation}
    \begin{bmatrix}
    x_1 \\
    x_2 \\
...
While the $W_1$ distance (Earth Mover's Distance) is not highly sensitive to outliers, its reliance on an $L_1$ absolute penalty introduces severe geometric pathologies 
