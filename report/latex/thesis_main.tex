...
Beyond temporal complexity, we must acknowledge the \textit{statistical sample complexity}. While our 1D projection approach maintains a good 1D convergence rate 
...
While the OT-ICA framework demonstrates excellent convergence on continuous distributions, real-world data often contains discrete signals, such as binary states, 
...
In contrast, the OT-ICA framework demonstrates good source separation until the trade-off between a fixed sample size 
...
Real-world signals rarely belong to a single statistical family. To test the algorithmic faculties of both OT-ICA and FastICA, 
...
By trading temporal efficiency for global geometric awareness, OT-ICA provides an excellent alternative capable of disentangling complex mixtures where traditional proxies fail.
...
This means the electrical signals generated by various brain regions, as well as non-brain muscles (such as the eyes and jaw), 
mix instantaneously and linearly before reaching the scalp electrodes. This physical reality perfectly satisfies the foundational linear mixing model of 
ICA: $\mathbf{X} = \mathbf{A}\mathbf{S}$.

Eye blinks (ocular artifacts) are notoriously problematic in neuroscience because their high amplitude completely masks the underlying cognitive brain waves. 
However, because eye blinks are mathematically highly sparse and "super-Gaussian", ICA has emerged as the gold-standard technique for isolating 
and removing them. In this application, we deploy the OT-ICA framework to a real-world clinical dataset to demonstrate its ability to blindly isolate sparse, 
super-Gaussian biological artifacts from continuous brain wave recordings.

\subsection{Empirical Results on Clinical EEG Data}
We utilized the \texttt{mne} library to fetch a real clinical MEG/EEG sample dataset. The raw continuous data was band-pass filtered between 1 Hz and 40 Hz, 
which is standard practice for ICA preprocessing to remove slow baseline drifts and high-frequency noise. We isolated a specific 10-second window 
(10.0s to 20.0s) known to contain massive eye-blinks across five frontal EEG channels. The data was standardized for numerical stability before applying 
the OT-ICA algorithm.

\begin{figure}[tbp]
    \centering
    \includegraphics[width=1.0\textwidth]{eeg_artifact_removal.png}
    \caption[EEG Artifact Removal via OT-ICA]{Application of OT-ICA to real clinical EEG data. \textbf{Top (Plot A):} 
    Five raw frontal EEG channels severely corrupted by high-amplitude eye-blink artifacts. \textbf{Bottom (Plot B):} 
    The independent components recovered by OT-ICA. The massive, synchronized spikes spanning all frontal channels have been perfectly 
    isolated into a single independent component (Comp 1), successfully cleaning the underlying continuous brain waves in the remaining components.}
    \label{fig:eeg_artifact_removal}
\end{figure}

As illustrated in Figure \ref{fig:eeg_artifact_removal}, the raw frontal electrodes (Plot A) are dominated by synchronized, 
high-amplitude spikes corresponding to the patient's eye blinks. After optimization on the Stiefel manifold, the OT-ICA framework successfully 
disentangled these signals. In the recovered components (Plot B), the entire ocular artifact has been mathematically compressed into 
a single independent component (Comp 1). The remaining components now represent the cleaned, continuous cognitive brain waves, 
free from the overwhelming ocular distortion. This confirms that the $W_2^2$ metric is highly effective at identifying 
and isolating sparse biological signals in real-world, noisy clinical environments.
...
\begin{table}[htbp]
    ...
    via the OT-ICA framework over 500 runs. \textbf{Takeaway:} OT-ICA accurately recovers the true structural parameters of the market.}
...
The distributions are tightly centered around the true parameter values, empirically confirming that OT-ICA effectively resolves the orthogonal ambiguity and recovers 
...
utilizing the excellent non-Gaussian recovery of OT-ICA to uniquely resolve orthogonal ambiguities in market price discovery. Ultimately, this framework establishes a new baseline for handling structurally heterogeneous data across the broader fields of signal processing and machine learning.

\section{Limitations}
Despite its geometric awareness, the OT-ICA framework is subject to specific constraints that delineate its operational boundaries. 
