...
The analytical behavior of this transport map is governed by Caffarelli's regularity theory \cite{caffarelli1992}. 
Caffarelli demonstrated that if both the source and target probability densities are 
...
when applied to finite empirical samples. We demonstrate why the squared $L_2$ penalty of the $W_2^2$ metric is mathematically requisite for gradient-based convergence.

\subsection{Empirical Gradients and Derivative Thrashing}
...
the resulting optimization landscape lacks the strict convexity required to smooth out finite sample noise. This is visually evident in the top panel, 
where the $W_1$ distance (blue curve) exhibits micro-level jaggedness compared to the smooth $W_2^2$ curve (orange).

The consequence of this lack of curvature is catastrophic for numerical solvers, as clearly illustrated in the bottom gradient profile. 
...
perpetually bouncing across local noise spikes and infinitely oscillating without ever cleanly converging on the true peak. 
Furthermore, this extreme volatility renders second-order Quasi-Newton approximations (such as L-BFGS) completely useless, 
as no stable Hessian can be approximated from such a shattered gradient.

In contrast, the squared penalty inherent to the $W_2^2$ metric heavily penalizes local geometric distortions, acting as a natural mathematical smoother. 
As shown in the gradient profile, the $W_2^2$ derivative acts as a highly stable, continuous sinusoidal wave. 
It gracefully and predictably decelerates precisely to zero as it reaches the global optimum. 
This robust retention of curvature allows gradient solvers to safely navigate the Stiefel manifold and settle into the true independent components, 
strictly justifying the selection of the squared Wasserstein distance for the OT-ICA framework.
...
via a mathematical operation known as a retraction \cite{absil2008optimization}.
...
its capacity to find global optima depends on computational resources in higher dimensions. Therefore, we evaluated both algorithms under two regimes: 
a "Low Compute" baseline (restricted random restarts for OT-ICA, and standard iteration limits for FastICA) and 
a "High Compute" regime (dense parallel restarts for OT-ICA, and higher convergence iterations allowed for FastICA).
...
In this section, we present two distinct mathematical blind spots that cause FastICA to fail on perfectly valid, non-Gaussian independent components. 
...
\subsection{Algorithmic Failure vs. Optimization Complexity}
...
\subsection{Structural Oscillation vs. Geometric Stability}
...
This thesis proposed and rigorously evaluated the Optimal Transport Independent Component Analysis (OT-ICA) framework, shifting the paradigm of linear source separation from local proxy approximations to global geometric matching. By formulating the search for independent components as a minimization of the squared Wasserstein distance ($W_2^2$) to a standard normal distribution on the Stiefel manifold, we addressed critical mathematical vulnerabilities inherent in industry-standard proxy algorithms.

Our theoretical analysis established that the squared Wasserstein distance strictly bounds mixture non-Gaussianity, providing a mathematically sound objective for extracting latent sources. Empirically, we demonstrated that while algorithms like FastICA fail catastrophically in the presence of specific structural blind spots—such as zero-negentropy topologies and vanishing curvature—OT-ICA consistently maintains algorithmic stability. By mapping the full cumulative distribution function rather than relying on point-estimates of density, OT-ICA proved uniquely capable of resolving highly complex, generalized hybrid mixtures where proxy solvers infinitely oscillate.

Furthermore, we introduced several critical algorithmic enhancements to make optimal transport computationally viable for ICA. These included the derivation of exact analytical Gaussian targets to eliminate discretization noise, batched vectorization for parallel hypothesis exploration, and the novel application of Gaussian dithering to prevent the piecewise linear shattering of discrete optimization landscapes. Finally, we demonstrated the practical utility of this framework in a financial econometrics setting, utilizing the robust non-Gaussian recovery of OT-ICA to uniquely resolve orthogonal ambiguities in market price discovery.

\section{Limitations}
Despite its geometric robustness, the OT-ICA framework is subject to specific limitations. The primary constraint is computational efficiency. Because calculating the exact Hessian of the Wasserstein distance requires continuous density estimation, OT-ICA cannot leverage the super-linear Newton-Raphson convergence that makes FastICA computationally instantaneous. Instead, OT-ICA relies on first-order Quasi-Newton methods (L-BFGS) combined with dense parallel random restarts. Consequently, the execution time scales exponentially with dimensionality. 

As a result, OT-ICA encounters a strict performance ceiling bounded by the curse of dimensionality. In our empirical evaluations with a fixed sample size of $N=10,000$, the expanding surface area of the Stiefel manifold caused the framework's separation accuracy to degrade severely beyond 40 dimensions, matching the universal statistical limits of linear ICA. 

Additionally, while Gaussian dithering successfully smooths step-wise empirical CDFs, purely symmetric discrete distributions (such as a sparse Binomial distribution) still yield intractable, terraced optimization landscapes. While OT-ICA excels in hybrid environments where continuous variables provide geometric lubrication, isolating pure, symmetrically discrete pools remains a significant hurdle for first-order gradients.

\section{Future Work}
The findings of this thesis open several promising avenues for future research. A natural progression is the integration of entropic regularization (Sinkhorn distances) to replace the Gaussian dithering step. While exact 1D Optimal Transport requires strict sorting, Sinkhorn divergence provides a naturally differentiable, smooth approximation of the Wasserstein metric that inherently handles discrete combinatorial traps without altering the underlying data.

Another critical direction involves improving the computational scaling of the framework. Investigating stochastic second-order methods, or utilizing neural-based transport map approximations (such as Normalizing Flows or optimal transport parameterized by neural networks), could drastically reduce the execution time required to search the Stiefel manifold. 

Finally, extending the geometric principles of $W_2$-based contrast functions into the realm of nonlinear Independent Component Analysis represents a highly compelling frontier. Because the Optimal Transport metric captures global structural deformations perfectly, it is uniquely positioned to regularize the highly unconstrained function spaces encountered when unmixing deep nonlinear representations.
