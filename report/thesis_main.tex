...
To further understand the behavior of these algorithms in non-continuous environments, we isolated specific discrete distributions to observe their individual impact on 
the optimization landscape. We generated mixtures containing one continuous Gaussian source and $(N-1)$ sources of a \textit{single} discrete type (Bernoulli, Poisson, or Binomial) 
evaluated at 30 and 40 dimensions.

\subsection{Performance on Pure Discrete Pools}
The empirical results revealed a nuanced performance profile for both algorithms depending on the specific statistical nature of the discrete distributions. 
FastICA proved highly capable of separating purely binary Bernoulli mixtures, successfully converging with a low Amari error. 
However, when faced with the multi-step, count-based geometries of Poisson and Binomial distributions, FastICA experienced complete algorithmic failure and could not separate 
the underlying sources.

Interestingly, the OT-ICA framework exhibited an identical performance dichotomy. Like FastICA, OT-ICA successfully unmixed the pure Bernoulli sources. 
The extreme bimodal geometry of the binary states, smoothed by the continuous Gaussian dithering step, provided a sufficient geometric signal for the optimal transport metric 
to establish reliable gradients. Conversely, OT-ICA failed completely when applied to the pure Poisson and Binomial pools. In these environments, 
the L-BFGS optimizer became predictably trapped in spurious local maxima, yielding catastrophic Amari errors ($> 1.0$). 

This shared limitation suggests that the failure may not be caused simply by the data being discrete, but rather by the specific parameterizations of these count-based distributions. 
The structural gaps within the Binomial and Poisson distributions under these parameters may fail to project a sufficiently strong non-Gaussian signal, 
depriving both proxy-based and optimal transport-based solvers of the necessary gradient information to separate the mixtures.

\subsection{The Necessity of Hybridization}
The uniform failure of OT-ICA on purely discrete pools raises a critical question: why utilize it over FastICA? The answer lies in the nature of real-world data, 
which is rarely purely discrete. As demonstrated by the preceding Generalized Hybrid experiment, FastICA fundamentally breaks when forced to navigate 
a "Full Hybrid" pool combining continuous and discrete variables. 

Conversely, OT-ICA thrives in precisely this hybrid environment. The presence of continuous distributions within a mixed pool provides sufficient smooth geometric structure 
to guide the $W_2$ metric. This continuous context allows the stochastic mini-batching and dithering mechanisms to effectively bridge the discrete gaps, 
enabling Optimal Transport to successfully disentangle complex mixtures where static proxies fail entirely.
