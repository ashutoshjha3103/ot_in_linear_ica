...
to a standard normal distribution on the Stiefel manifold, we addressed critical mathematical vulnerabilities inherent in industry-standard proxy algorithms.
...
utilizing the robust non-Gaussian recovery of OT-ICA to uniquely resolve orthogonal ambiguities in market price discovery.
...
The primary limitation is \textbf{computational efficiency}. Because the exact Hessian of the Wasserstein distance requires continuous density estimation, 
OT-ICA cannot leverage the super-linear Newton-Raphson convergence that makes FastICA computationally instantaneous. Instead, the framework relies on first-order 
...
Furthermore, the framework encounters a strict \textbf{statistical performance ceiling} dictated by the curse of dimensionality. While 1D Optimal Transport is statistically robust, 
the global search for the unmixing matrix is bound by the statistical resolution of the joint distribution. As the dimensionality of the Stiefel manifold expands, 
the 'volume' of the search space increases, and the $O(N \log N)$ sorting cost is compounded by the necessity for $K$ restarts to avoid local minima. 
...
matching the universal statistical limits of linear ICA.

Finally, there remains a \textbf{geometric bottleneck} regarding purely discrete distributions. While Gaussian dithering successfully smooths step-wise empirical CDFs, 
purely symmetric and sparse discrete distributions (such as a low-$n$ Binomial distribution) still yield intractable, "terraced" optimization landscapes with zero-gradient plateaus. 
...
a significant hurdle for first-order gradient solvers. Consequently, OT-ICA should be viewed as a high-precision tool for complex, heterogeneous mixtures rather than 
a general-purpose substitute for high-speed, high-dimensional applications.
...
or utilizing neural-based transport map approximations (such as Normalizing Flows or optimal transport parameterized by neural networks), 
could drastically reduce the execution time required to search the Stiefel manifold. 
