...
Mathematically, this operation convolves the discrete Dirac spikes of 
the empirical distribution with a narrow Gaussian kernel. This effectively "melts" the rigid staircase of the discrete CDF into a strictly increasing, 
...
To empirically evaluate the success of an Independent Component Analysis algorithm, we require a quantitative metric that measures the distance between 
the estimated unmixing matrix $\mathbf{W}_{\text{est}}$ and the true generative mixing matrix $\mathbf{A}$. Because ICA is subject to inherent scaling and permutation ambiguities, 
standard matrix distance metrics (such as the Frobenius norm) are entirely unsuitable. 

Instead, we utilize the industry-standard Amari Performance Index, often referred to as the Amari error \cite{amari1996new}. 
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lp{10cm}@{}}
        ...
        \addlinespace
        $E \ge 1.0$ & Catastrophic algorithmic failure. The estimated matrix is effectively a random orthogonal projection with no meaningful relation to the true sources. \\
        \bottomrule
    \end{tabular}
