...
In stark contrast, the W2-ICA framework demonstrates robust source separation. By completely bypassing parametric approximations and instead mapping the full global geometry of the empirical Cumulative Distribution Function (CDF) via Optimal Transport, W2-ICA easily detects the non-Gaussian structure of the Phantom Gaussian. 

While W2-ICA experiences minor performance degradation in the Low Compute regime at $d=25$, this is a symptom of the exponentially expanding, highly non-convex surface area of the Stiefel manifold, not a failure of the objective function. When provided with dense parallel restarts in the High Compute regime, W2-ICA successfully navigates the complex search space and fully restores accurate unmixing ($E < 0.3$). This empirically reaffirms that while W2-ICA is bound by computational search complexity, it fundamentally resolves the catastrophic mathematical blind spots that plague FastICA.
...
To isolate fundamental mathematical failures from standard optimization difficulties, we evaluated both FastICA and W2-ICA across increasing dimensions ($d \in [5, 25]$) at a fixed sample size of $N=10,000$. The experiment was conducted under two distinct computational regimes: a "Low Compute" baseline and a "High Compute" environment (characterized by maximum iterations extended to $10,000$ for FastICA, and massive parallel random restarts spanning up to $600$ vectors for W2-ICA).

\begin{figure}[tbp]
    \centering
    \includegraphics[width=1.0\textwidth]{vanishing_curvature_amari_compute.png} 
    \caption[Performance on Trimodal Failure Mode]{Amari error comparison between FastICA and W2-ICA across increasing dimensions for the engineered Trimodal distribution. FastICA experiences catastrophic mathematical failure ($E > 1.0$) across all regimes. W2-ICA's performance degradation at higher dimensions is revealed to be a non-convexity search problem, as the High Compute regime successfully restores good separation.}
...
Conversely, W2-ICA's behavior reveals a completely different limitation. Under the Low Compute regime, W2-ICA successfully unmixed lower dimensions but degraded as $d$ increased. However, when shifted to the High Compute regime, W2-ICA's performance recovered, maintaining a "Good Separation" Amari error ($E < 0.3$) up through 25 dimensions. 

This proves that W2-ICA does not suffer from the curvature trap. Because it evaluates the entire global geometry of the empirical CDF via Optimal Transport, it is mathematically immune to local density blind spots. Its performance ceiling is instead dictated purely by the \textit{Curse of Dimensionality} and non-convexity. As dimensions increase, the surface area of the Stiefel manifold expands exponentially, becoming littered with shallow local maxima. W2-ICA succeeds mathematically, but extracting that success requires sufficient computational power (dense random restarts) to thoroughly search the highly non-convex optimization landscape.
...
%as W2-ICA relies on first-order global CDF matching rather than second-order local density approximations.

\section{The Generalized Hybrid Mixture Stress Test}
Real-world signals rarely belong to a single statistical family. To test the true algorithmic robustness of both W2-ICA and FastICA, we designed a highly complex, 
generalized hybrid mixture. This stress test evaluates the capacity of the algorithms to separate sources drawn from fundamentally contradictory geometric landscapes.

\subsection{Experimental Setup}
We generated linear mixtures spanning $10$ to $75$ dimensions, maintaining a fixed sample size of $10,000$. To satisfy the ICA identifiability constraint, 
exactly one source was fixed as a standard Gaussian distribution, $\mathcal{N}(0,1)$. The remaining $(d-1)$ independent sources were randomly drawn from 
a diverse pool of eight distinct distributions: Laplace, Bernoulli, Uniform, Student-t, Poisson, Binomial, Chi-squared, and Exponential. 
To account for the unpredictable presence of discrete and light-tailed components within this hybrid pool, W2-ICA utilized the robust settings engineered in 
Chapter 4 ($\sigma_{\text{dither}} = 0.01$, stochastic batch size $= 1024$).

\subsection{Results: Structural Oscillation vs. Geometric Stability}
The results revealed a stark divergence in the stability of the two algorithms. The W2-ICA framework successfully maintained a low Amari error ($< 0.25$) up to $35$ dimensions. 
This performance validates that the globally-aware $W_2$ metric, when smoothed via dithering and regularized via mini-batching, generalizes robustly across highly diverse topological landscapes.

In contrast, FastICA's performance degraded rapidly, crossing the critical $0.5$ Amari error threshold immediately after $20$ dimensions. During the generalized experiment, 
FastICA consistently triggered non-convergence warnings. We hypothesized that this failure stems from the Newton-Raphson solver's assumption of uniform curvature. 
When forced to simultaneously evaluate highly kurtotic signals (e.g., Laplace) and negatively kurtotic signals (e.g., Uniform), 
the fixed-point update steps generate contradictory gradient information, causing the solver to oscillate indefinitely. 

To confirm this was a structural oscillation and not merely a matter of insufficient compute time, we evaluated FastICA under extreme optimization parameters 
(up to $10,000$ maximum iterations) and relaxed convergence tolerances ($1e-3$). Even under these highly permissive conditions, FastICA's convergence rate plummeted 
to $0\%$ past 30 dimensions. This definitively proves that in unpredictable, hybrid environments, proxy-based Newton solvers become trapped in contradictory gradient loops, 
whereas W2-ICA's first-order global CDF matching remains stable.
...
Interestingly, the W2-ICA framework exhibited an identical performance dichotomy. Like FastICA, W2-ICA successfully unmixed the pure Bernoulli sources. 
...
to establish reliable gradients. Conversely, W2-ICA failed completely when applied to the pure Poisson and Binomial pools. In these environments, 
...
The uniform failure of W2-ICA on purely discrete pools raises a critical question: why utilize it over FastICA? The answer lies in the nature of real-world data, 
...
Conversely, W2-ICA thrives in precisely this hybrid environment. The presence of continuous distributions within a mixed pool provides sufficient smooth geometric structure 
...
For each simulation, the W2-ICA framework successfully recovered the independent structural shocks. Figure \ref{fig:shock_recovery} illustrates a representative run, demonstrating the high correlation between the true generative shocks and the empirical shocks estimated by W2-ICA. The tight alignment along the identity line confirms that the algorithm successfully unmixed the latent variables prior to any economic transformation.

\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.95\textwidth]{shock_recovery_scatter.png}
    \caption[Recovery of Structural Shocks via W2-ICA]{Pairplot demonstrating the recovery of true structural shocks versus the estimated shocks from W2-ICA for a single Monte Carlo run. \textbf{Takeaway:} The strong linear alignment indicates highly accurate unmixing of the latent non-Gaussian market innovations.}
...
Table \ref{tab:simulated_is_results} presents the aggregate results across all 500 simulations. The W2-ICA estimator, combined with our economic identification strategy, demonstrates exceptional accuracy. The mean estimated information shares converge almost exactly to their theoretical targets, with minimal standard deviations indicating a highly stable recovery process. 
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}lccc@{}}
        ...
        \bottomrule
    \end{tabular}
    \caption[Simulation Results for Information Share Recovery]{Monte Carlo simulation results comparing true Information Shares (IS) against the estimated IS recovered via the W2-ICA framework over 500 runs. \textbf{Takeaway:} W2-ICA robustly and accurately recovers the true structural parameters of the market.}
    \label{tab:simulated_is_results}
\end{table}

Furthermore, Figure \ref{fig:is_distribution} illustrates the empirical density of the estimated information shares across all simulation runs. The distributions are tightly centered around the true parameter values, empirically confirming that W2-ICA effectively resolves the orthogonal ambiguity and robustly recovers the structural drivers of price discovery.

\begin{figure}[tbp]
    \centering
    \includegraphics[width=0.85\textwidth]{econ_applicatn_is_distribution.png} % Replace with your actual filename
